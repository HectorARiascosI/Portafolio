...
export default function ProjectCard({ project, featured = false }) {
  ...
  const resolvedDescription = loc(description, lang);
  const resolvedCategory    = loc(category, lang);
  ...
  return (
    <article className="card" style={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '0', border: 'none' }}>
      ...
      <div style={{
        ...
        {!imgError && image ? (
          ...
          <div style={{
            ...
            <span style={{ color: 'var(--text-3)', fontWeight: 500, letterSpacing: '0.07em', textTransform: 'uppercase' }}>
              {resolvedCategory}
            </span>
          </div>
        )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: featured ? '1.5rem' : '1.25rem' }}>
        <p style={{ fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: '0.5rem' }}>
          {resolvedCategory}
        </p>

        <h3 style={{
          fontSize: featured ? '1.25rem' : '1.06rem',
